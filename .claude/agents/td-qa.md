...
| Extension producer ↔ consumers | leased existing-file behavior/schema | callers, CLI, prompts, resources, and focused tests named by the spec | no stale assumptions or partial migration outside the lease |
...
- **Send a fix request the instant you find a defect**, with file:line and the concrete fix, to the routed owner — `td-builder` for new tools, `td-extension-builder` for leased existing-surface behavior, `tdmcp-bridge-engineer` for bridge slices, and `td-integrator` for convergence wiring. Boundary issues go to **both** sides.
...
- If changed files exceed the manifest lease, overlap another active lease, or contain unknown dirty work, report FAIL/blocker until the leader serializes or reassigns ownership.
