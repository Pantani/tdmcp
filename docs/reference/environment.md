...
| `TDMCP_TOOL_PROFILE` | `full` | Tool exposure profile. `full` registers every tool; `safe` additionally hides the destructive/raw-code tools (`delete_td_node`, `create_panic`, `manage_checkpoint`, `manage_component`, `execute_python_script`, `exec_node_method`) — a strict superset of `TDMCP_RAW_PYTHON=off`. Use `safe` for an autonomous in-TD agent (e.g. dotsimulate's LOPs MCP Client). Default `full` keeps existing clients unchanged. |
