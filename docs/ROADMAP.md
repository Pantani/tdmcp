...
- 🧪 **Creative RAG MVP (opt-in, experimental).** A local-only creative
  ...
  three confirmed museum sources (Art Institute of Chicago, The Met,
  Rijksmuseum), in-memory cosine over a local JSONL index, embeddings via local
  Ollama. **Repertório, não policy** — no bridge, DMX, or Python exec.
  Follow-ups: a `TDMCP_RAG_BACKEND=lancedb` vector store and the 10 documented
  stub sources (Europeana, Wikimedia/Wikidata, Smithsonian, Cleveland, Harvard,
  Cooper Hewitt, Internet Archive, WikiArt, portfolios, Shadertoy). See
