...
With v0.8.3 current locally and v0.8.x still the active stabilization line, most
of the road to a tagged 1.0 remains **consolidation** (API stability, coverage,
recipes, bridge hardening, docs, one-click install), tracked as measurable gates
in [v1.0.0 — Consolidation](#v100-consolidation) below. One new design-stage
slice is now planned: a Soundcraft Ui24R / mixer-aware expansion of
AI-Controlled Party. It belongs here because it needs future code changes, but
it is **not shipped** and **not a live hardware claim**. Hardware-, live-music-,
multimodal-LLM- and GPU/CUDA-gated items remain in **Out of scope (for now)**.
Version targets are a rough sequence, **not a promise**. The exhaustive,
item-by-item backlog lives in the [planning archive](#full-backlog).
...
landed in the v0.8 release line, the baseline road to 1.0 is still
consolidation, not broad new tool surface. The Ui24R mixer-scene work is a
bounded AI Show Director extension and must ship behind dry-run, manifest and
bench-validation gates before any live adapter is considered. The measurable
1.0 gates live in [v1.0.0 — Consolidation](#v100-consolidation) below.
