...
/**
 * True when the exec path is unavailable because raw Python exec is disabled or
 * missing on the bridge (as opposed to a real node/script error we should surface).
 */
function isExecUnavailable(err: unknown): boolean {
  const msg = err instanceof Error ? err.message : String(err);
  return /exec/i.test(msg) && /(disabl|not allow|forbidden|unavailable|missing)/i.test(msg);
}
