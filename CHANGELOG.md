...
- Add an opt-in reactive tool-call rate gate (`TDMCP_RATE_LIMIT_SLOW_MS` /
  `TDMCP_RATE_LIMIT_COOLDOWN_MS`): when a tool call exceeds the slow threshold,
  the next call waits out the cooldown so agent bursts don't stall TouchDesigner.
  Off by default (both env values must be > 0 to activate); a half-configured
  gate is disabled and warned about. Pattern inspired by Derivative's official
  TouchDesigner TDMCP `request_gate.py` (Shared Use License); reimplemented in
  Node/TS.
...
- Harden the TouchDesigner bridge request handler with a loopback-only address
  ...
  environment.
- Reject `POST`/`PATCH`/`PUT` bodies whose `Content-Type` is present but not
  `application/json`, blocking simple cross-site form/`fetch` POSTs to the bridge
  (CSRF / DNS-rebinding protection alongside the existing `Origin`/`Host` guards).
