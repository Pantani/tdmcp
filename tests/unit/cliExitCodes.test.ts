...
import {
  detectInlineProtocol,
  renderInlineImage,
} from "../../src/cli/inlineImage.js";
...
describe("exit-code taxonomy", () => {
  ...
  it("classifies a timeout as TD offline (3)", () => {
    expect(classifyTdErrorExit("TouchDesigner request timed out after 5000ms (POST /api/exec).")).toBe(
      ExitCode.TdOffline,
    );
  });

  it("classifies raw socket errors as TD offline (3)", () => {
    expect(classifyTdErrorExit("fetch failed: ECONNREFUSED 127.0.0.1:9980")).toBe(ExitCode.TdOffline);
    expect(classifyTdErrorExit("socket hang up")).toBe(ExitCode.TdOffline);
  });

  it("classifies a reached-but-failed op as TD error (4)", () => {
    expect(classifyTdErrorExit("get_inline_preview failed at /project1/top1: Not found: /project1/top1")).toBe(
      ExitCode.TdError,
    );
    expect(classifyTdErrorExit("No such parameter 'wibble' on /project1/noise1")).toBe(ExitCode.TdError);
  });
...
describe("renderInlineImage", () => {
  ...
  it("emits an honest ascii fallback (never raw base64 into a pipe)", () => {
    const out = renderInlineImage(b64, { protocol: "ascii", width: 64, height: 64, mimeType: "image/png" });
